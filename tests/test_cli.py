...
""",
    ...
    nested.mkdir()
    assert denial(
        invoke(workspace, tool(workspace, command="git status", workdir=str(nested)))
    ) == ("Blocked by guard")
...
def test_claude_approval_without_turn_id(workspace: Path):
    ...
    del payload["turn_id"]
    allowed(
        invoke(workspace, {**payload, "prompt": "I insist"}, "user-prompt-submit", harness="claude")
    )
    allowed(invoke(workspace, payload, harness="claude"))
    assert (
        invoke(
            workspace, {**payload, "source": "compact"}, "session-start", harness="claude"
        ).returncode
        == 0
    )
    allowed(invoke(workspace, payload, harness="claude"))
    assert (
        invoke(
            workspace, {**payload, "source": "resume"}, "session-start", harness="claude"
        ).returncode
        == 0
    )
    assert denial(invoke(workspace, payload, harness="claude")) == "Blocked by guard"
