...
Validation must reject malformed values before core behavior sees them. `NewType`
adds static separation but does not validate data, so use it only when confusing two
primitive domain values would cause a bug.

## Prefer composition

Combine focused functions and values when behavior varies. Use inheritance only for
genuine is-a relationships or framework contracts. Do not add interfaces or
factories with only one implementation.

## Couple duplicated documentation

When prose repeats a code-owned value whose drift would mislead users, add a `NOTE:`
